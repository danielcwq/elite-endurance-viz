...
The rebuildable source release combines the manifested legacy repository CSV snapshot with `data/raw/2024/reconciled/20260902T190000Z/production_only_activities.csv`, the immutable subset recovered from the sealed Mongo snapshot. `data/reference/athlete_registry_2024.csv`, `data/reference/athlete_external_accounts_2024.csv`, configuration under `config/`, and schema under `schema/` are versioned control inputs.
...
The live Mongo collections, `cleaned_athlete_metadata.csv` aggregate columns, `data/metadata/athlete_statistics.csv`, notebooks, raw batch summaries, and timestamped backups are provenance-only. The full immutable Mongo export is retained offline and represented by a committed manifest; only its committed reconciliation supplement participates in builds. Analytical queries must use canonical performance, activity, coverage, weekly, and athlete-summary tables instead.
