...
class ServingRepositoryTests(unittest.TestCase):
    def test_search_uses_stable_id_and_activity_query_is_paginated(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            countries = repository.map_countries()
            self.assertEqual(countries[0]["nationality_code"], "CAN")
            self.assertEqual(countries[0]["athlete_count"], 1)
            self.assertEqual(countries[0]["observed_athlete_count"], 1)
            roster = repository.map_country_athletes("can")
            self.assertEqual(roster[0]["athlete_id"], result[0]["athlete_id"])
