...
def comparison_page(repository,banner,first='800m',second='5000m',metric='distance',view='distribution',sex='both'):
    ...
    return (*metadata('Compare recorded running — EnduranceViz 2024'+(' · Synthetic Demo' if synthetic else ''),
        ...
        Details(Summary('Cohort counts, missing values, and middle 50%'),
            Div(Table(Thead(Tr(*(Th(x,scope='col') for x in ('Cohort','Registry','Contributors','No plotted value',f'Middle 50% ({unit}/week)','With source warnings')))),
                      Tbody(*(counts_row(g) for g in groups))),cls='ev-table-scroll',tabindex='0',role='region',aria_label='Cohort denominators'),
            P('Registry counts all athletes assigned to the event and recorded-sex group. Contributors have a value for this chart. No plotted value is the remaining registry count, not a count of athletes rejected for low mileage.'),
            P('A value is unavailable when there are no qualifying recorded-running weeks'+(' or, in this view, no stored assigned-event score.' if view=='points' else '.')+' For distance, a week qualifies only when every stored Run has a usable distance. Weeks without recorded Runs are not treated as zero training. No minimum number of weeks or P0 eligibility filter is applied.'),
            P('Middle 50% spans the 25th to 75th percentiles of the plotted athlete medians: the central half of athlete values. It is not a confidence interval, an accuracy estimate, or a recommended training range.'),
